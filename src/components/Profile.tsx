...
import { fetchAllCoaches } from '../services/userData';
import { Save, User, Ruler, Camera, Target, Link, Search, CheckCircle, Clock, Info, Loader2 } from 'lucide-react';
...
const Profile: React.FC<Props> = ({ profile, onUpdate, onConnectRequest }) => {
  ...
  // Real Data for Coaches
  const [activeCoaches, setActiveCoaches] = useState<UserProfile[]>([]);
  const [isLoadingCoaches, setIsLoadingCoaches] = useState(false);

  useEffect(() => {
      if (connectMode === 'SEARCH' && activeCoaches.length === 0) {
          setIsLoadingCoaches(true);
          fetchAllCoaches()
            .then(data => setActiveCoaches(data))
              ...
              {profile.coachConnectStatus === 'Connected' && (
                  <div className="absolute inset-0 bg-emerald-900/90 flex items-center justify-center z-10">
                      <div className="text-center">
                          ...
                          <p className="text-emerald-300 mt-2 font-medium">مربی شما: {profile.connectedCoachName || 'نامشخص'}</p>
                ...
                <div className="flex gap-4 items-end animate-fade-in">
                 ...
                 <div className="space-y-4 animate-fade-in">
                     ...
                     <div className="max-h-60 overflow-y-auto custom-scrollbar space-y-2 border border-slate-700/50 rounded-lg p-2 bg-slate-900/30">
                         {isLoadingCoaches ? (
                             <div className="text-center p-4"><Loader2 className="animate-spin text-blue-500 mx-auto" /></div>
                         ) : filteredCoaches.length === 0 ? (
                             <div className="text-center text-slate-500 text-xs p-4">
                                {searchQuery ? 'مربی‌ای با این نام یافت نشد.' : 'لیست مربیان فعال در حال حاضر خالی است.'}
                             </div>
                         ) : (
